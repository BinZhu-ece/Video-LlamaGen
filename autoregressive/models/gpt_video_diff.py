...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        if targets_video is not None and video_latent is not None:
            ...
            visual_token_mask = torch.ones(video_latent.shape[:2], device=video_latent.device) # all tokens need to be culculated loss
            input_videos_latent = video_latent[:, :-1]
            
            self.freqs_cis = self.freqs_cis.to(video_latent.device)
            # concat text embedding and video latent embeddings ( from Causual Video VAE)
